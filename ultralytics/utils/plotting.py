...
def feature_visualization(x, module_type, stage, save_dir=Path("runs/detect/exp")):
    ...
    if isinstance(x, torch.Tensor):
        _, channels, height, width = x.shape  # batch, channels, height, width
        if height > 1 and width > 1:
            f = save_dir / f"stage{stage}_{module_type.split('.')[-1]}_feature_single.png"  # filename

            feature = x[0, 0].cpu().numpy()  # select batch index 0, channel 0
            plt.imshow(feature, cmap='gray')
            plt.axis("off")

            LOGGER.info(f"Saving {f}... (1/{channels})")
            plt.savefig(f, dpi=300, bbox_inches="tight")
            plt.close()
            np.save(str(f.with_suffix(".npy")), x[0].cpu().numpy())  # npy save
