...
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, autopad, CBAM, ChannelAttention
...
class C2f_Max_Pooling(nn.Module):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""
    ...
    def forward(self, x):
        ...
        y = list(self.cv1(x).chunk(2, 1))
        y.extend(m(y[-1]) for m in self.m)
        return self.max_pooling(self.cv2(torch.cat(y, 1)))
...
class C2f_Fractional_Max_Pooling(nn.Module):
    """Faster Implementation of CSP Bottleneck with 2 convolutions."""
    ...
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        ...
        self.cv2 = Conv((2 + n) * self.c, c2, 1)  # optional act=FReLU(c2)
        self.m = nn.ModuleList(Bottleneck(self.c, self.c, shortcut, g, k=((3, 3), (3, 3)), e=1.0) for _ in range(n))
        self.max_pooling = nn.FractionalMaxPool2d(2, output_ratio=(0.75, 0.75), _random_samples=1)
