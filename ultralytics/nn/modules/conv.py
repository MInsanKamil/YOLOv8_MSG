...
class Conv_Spatial_Max_Pooling(nn.Module):
    """Standard convolution with args(ch_in, ch_out, kernel, stride, padding, groups, dilation, activation)."""
...
class Conv_Spatial_Max_Pooling_Dropout(nn.Module):
    """Standard convolution with args(ch_in, ch_out, kernel, stride, padding, groups, dilation, activation)."""
...
class Conv_Attn_Avg_Pooling(nn.Module):
    """Standard convolution with args(ch_in, ch_out, kernel, stride, padding, groups, dilation, activation)."""
    ...
    def __init__(self, c1, c2, k=1, s=1, km=3, p=None, g=1, d=1, act=True):
        ...
        self.bn = nn.BatchNorm2d(c2)
        self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()
        self.avg_pool = nn.AvgPool2d(km, stride=2)  # GAP layer
        self.cbam= CBAM(c2)
...
class Conv_Avg_Attn_Pooling(nn.Module):
    """Standard convolution with args(ch_in, ch_out, kernel, stride, padding, groups, dilation, activation)."""
...
class GhostConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, kd=7, g=1, act=True):
        ...
        self.cv2 = Conv(c_, c_, kd, 1, None, c_, act=act)
